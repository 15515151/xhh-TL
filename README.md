...
**过码**：默认是「发链接、用户手划」。想要**全自动**（自己解滑块、不用人管）需自行部署过码
服务——见仓库 **`solver` 分支**的 `service/geetest/`，部署好把 `auto_verify_addr` 填成
`http://127.0.0.1:8766/solve` 即可；没配或服务没起时，自动回退到手动链接，不会报错。

**撞码会自动提醒**：本插件（含 genshin、miao-plugin 等走米游社的指令）一撞验证码，就会在群里
@ 你提示发 `#过码`，不用自己去猜「为什么查不出来」。
