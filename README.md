...
- **两种卡片样式**（`tl_card_style`）：
  - **立绘卡**（`portrait`）：原神 / 星铁 / 绝区零均为右侧大立绘样式；绝区零角色图默认取自 `resources/zzzrole`（可配置 `tl_zzz_portrait_folder`）
  - **桌面小组件卡**（`widget`）：仿官方桌面小组件的竖卡，不使用立绘底图；底部可显示「限时活动」区块（活动名 + 进度 + 倒计时）
    - 星铁 / 绝区零：活动数据来自体力 widget 接口自带字段，与官方小组件同源，**零额外请求**
    - 原神：widget 接口不返回活动，插件用完整 CK 额外拉取官方 `act_calendar`（限时活动 + 渊月螺旋 / 剧诗等固定活动）；无 CK 或拉取失败时静默跳过，不影响体力主流程
    - 开关：`tl_widget_activity`（默认开），条数：`tl_widget_activity_limit`（默认 4）
- **回复规则**：单图结果引用触发消息；多图合并转发**不引用**；相关「正在…」进度提示约 30 秒后自动撤回

### 体力阈值推送
- 每个用户在**群里**各自设定阈值，体力恢复到阈值（含）以上时，机器人在该群 **@你** 并发送体力卡片
- 原神 / 星铁 / 绝区零**分开**：分开指令、分开阈值、分开推送
  - 原神看**原粹树脂**（`current_resin`）：`#原神体力推送 130`
  - 星铁看**开拓力**（`current_stamina`）：`#星铁体力推送 200`
  - 绝区零看**电量**（`energy.progress.current`）：`#绝区零体力推送 220`
- **全 id 推送**：`#原神体力全推送 130` / `#星铁体力全推送 200` / `#绝区零体力全推送 220` —— 把该账号名下**全部 UID** 分别监控、分别推送（普通推送只盯主 UID）
- 关闭：`#原神体力推送关闭` / `#星铁体力推送关闭` / `#绝区零体力推送关闭`（全推送加「全」后缀，如 `#原神体力全推送关闭`）
- 查看订阅：`#体力推送列表`
- **只提醒一次**：达到阈值推送后进入静默，体力回落到阈值以下后自动重新监控，下次再满再提醒
- 默认每 10 分钟检查一次（`resin_push_cron` 可调，不建议太频繁以免米游社风控）；订阅存于 `data/resin_push.json`
- 复用体力查询与出图，无需额外绑定；开关：`resin_push_enable`（锅巴可配）
...
- `apps/TL.js` - 体力查询
- `apps/resinPush.js` - 体力阈值推送（达阈值在群 @用户 发图；原神/星铁/绝区零分开，各支持主 UID 与全 id 推送）
